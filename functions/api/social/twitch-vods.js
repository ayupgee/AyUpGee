/**
 * GET /api/social/twitch-vods
 *
 * Returns the latest 3 Twitch VODs from the D1 social_posts table.
 * Rows are written by the ayupgee-social-sync Cloudflare Worker (every 12 h).
 * No Twitch credentials are needed here — this is a pure D1 read.
 *
 * ─── Required env vars ─────────────────────────────────────────────────────────
 ...
 *
 * ─── Caching ────────────────────────────────────────────────────────────────────
 *   2-hour edge cache — the sync worker refreshes D1 every 12 h so this is plenty.
 */
...
export async function onRequestGet(context) {
  ...
  try {
    ...
    console.error('[/api/social/twitch-vods]', e.message);
    return fail('Failed to fetch Twitch VODs');
  }
}
